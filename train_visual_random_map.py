...
try:
    ...
    raise ImportError("无法找到 'robot_visual_env_random_map.py'。请确保该文件与训练脚本在同一目录下，或已添加到 PYTHONPATH 中。")

# 1. 全局配置中心 (必须放在 import matplotlib 之前！)
class Config:
    # --- 训练设置 ---
    num_envs = 1                # 调试时设为1，正式训练建议 4~16
    total_timesteps = 40_000_000 
    lr = 3e-4                   # 注意：下方的 SAC 实例化中我们对齐了固定地图的 1e-4
    seed = 42                   
    ...
    resume_from = None
    # resume_from = "/home/iansten/code/IsaacLabExtensionTemplate/sac_lidar_logs_random/sac_lidar_interrupted.zip"
    
    # --- 路径设置 ---
    # 请确保路径正确 (使用了新的随机地图对应的 xml)
    ...
    log_dir = "./sac_lidar_logs_random/" # 建议随机地图用独立的日志文件夹
...
# 4. 自定义网络结构 (Grid Map CNN + State GRU) - 【完全对齐固定地图】
...
class FailureAnalysisCallback(BaseCallback):
    ...
    def _plot_death_map(self, counts):
        ...
        # 随机地图是 20x20，这里的散点图范围设为 -10 到 10
        ax.set_xlim(-10, 10)
        ax.set_ylim(-10, 10)
        ax.set_title(f"Termination Map (Step {self.num_timesteps}) - Random Maps")
        ax.legend()
        ax.grid(True)
...
def main():
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="sac_lidar_random",
        save_replay_buffer=False,
        verbose=1
    )

    fail_analysis_callback = FailureAnalysisCallback(save_dir=Config.log_dir, check_freq=20000000)
    analysis_callback = DetailedRewardAnalysisCallback(log_freq=500)
    ...
    if Config.resume_from and os.path.exists(Config.resume_from):
        ...
        model = SAC.load(Config.resume_from, env=env, learning_rate=lr_schedule, device="cuda", custom_objects={"policy_kwargs": policy_kwargs})
    else:
        print("--> 初始化全新 SAC 模型 (With Lidar GridMap + GRU for Random Maps)")
        model = SAC(
            "MultiInputPolicy",
            env,
            ...
            learning_rate=2e-5,      # 【关键修复】这里强制统一为 1e-4，和固定地图保持一致
            buffer_size=6500_000,
            batch_size=1024,
            gamma=0.993,
            tau=0.0005,
            ent_coef='auto',
            # ent_coef=0.04,         # 保留这个注释，如果你发现在随机地图探索不够，可以取消注释固定为0.04
            train_freq=(100, "step"), 
            gradient_steps=15,       
    ...
    try:
        model.learn(
            ...
            callback=callbacks,
            tb_log_name="SAC_Lidar_Random_Map",
