...
# Diffusion Policy Model & Planner Definitions
...
class DiffusionTrajectoryPlanner:
    def __init__(self, device='cuda', checkpoint_path=None, horizon=60): # horizon 建议设为训练时的长度(如20)
        self.device = device
        self.horizon = horizon
        self.action_dim = 2 # (x, y)
        self.num_diffusion_iters = 50 # 必须与训练时的步数一致
        
        # 1. 模型组件
        # 注意：MapEncoder 的输入维度必须与 train_diffusion.py 中定义的一致
        self.map_encoder = MapEncoder(input_channels=1, feature_dim=128).to(device)
        self.noise_pred_net = ConditionalUnet1D(input_dim=2, cond_dim=128+2).to(device) 
        
        # 2. 噪声调度器 (DDPM Constants)
        # [修正] linspace 的步数必须等于 num_diffusion_iters
        self.beta_schedule = torch.linspace(0.0001, 0.02, self.num_diffusion_iters).to(device)
        self.alpha_schedule = 1 - self.beta_schedule
        self.alpha_cum_schedule = torch.cumprod(self.alpha_schedule, dim=0)
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            print(f"Warning: No diffusion weights found at {checkpoint_path}! Planner will output noise.")

    def plan(self, local_map_np, current_pos_world, current_yaw, goal_pos_world):
        """
        生成轨迹
        :param local_map_np: 局部地图 (H, W)
        :param current_pos_world: 机器人世界坐标 [x, y]
        :param current_yaw: 机器人朝向 (弧度)
        :param goal_pos_world: 目标点世界坐标 [x, y]
        :return: 世界坐标系下的轨迹 (Horizon, 2)
        """
        # 1. 准备地图数据
        local_map_tensor = torch.from_numpy(local_map_np).float().unsqueeze(0).unsqueeze(0).to(self.device) # (1, 1, H, W)
        
        # 2. 坐标转换 (World -> Local Robot Frame)
        # 计算相对于机器人的目标点位置
        dx = goal_pos_world[0] - current_pos_world[0]
        dy = goal_pos_world[1] - current_pos_world[1]
        
        c, s = np.cos(current_yaw), np.sin(current_yaw)
        
        # 旋转矩阵 (逆时针旋转 -yaw，将世界向量转到机器人朝向)
        local_goal_x = c * dx + s * dy
        local_goal_y = -s * dx + c * dy
        
        # [修正] 归一化：除以 10.0 (匹配训练逻辑)
        relative_goal = torch.tensor([[local_goal_x, local_goal_y]], device=self.device).float() / 10.0

        with torch.no_grad():
            # 3. 编码条件
            map_feat = self.map_encoder(local_map_tensor) # (1, 128)
            global_cond = torch.cat([map_feat, relative_goal], dim=-1) # (1, 130)
            
            # 4. 初始化噪声轨迹
            noisy_trajectory = torch.randn((1, 2, self.horizon), device=self.device)
            
            # 5. 逐步去噪
            for t in reversed(range(self.num_diffusion_iters)):
                t_tensor = torch.tensor([t], device=self.device).long()
                
                noise_pred = self.noise_pred_net(noisy_trajectory, t_tensor, global_cond)
                
                alpha = self.alpha_schedule[t]
                alpha_cum = self.alpha_cum_schedule[t]
                beta = self.beta_schedule[t]
                
                if t > 0:
                    noise = torch.randn_like(noisy_trajectory)
                else:
                    noise = torch.zeros_like(noisy_trajectory)
                
                # DDPM Update Step
                noisy_trajectory = (1 / torch.sqrt(alpha)) * (
                    noisy_trajectory - ((1 - alpha) / (torch.sqrt(1 - alpha_cum))) * noise_pred
                ) + torch.sqrt(beta) * noise
            
            # 6. 后处理与坐标还原
            # (1, 2, T) -> (T, 2)
            local_traj_np = noisy_trajectory.squeeze(0).permute(1, 0).cpu().numpy()
            
            # [修正] 反归一化：乘以 10.0
            local_traj_np = local_traj_np * 10.0
            
            # 7. 坐标转换 (Local Robot Frame -> World)
            # 旋转回世界坐标系并平移
            # x_world = c * x_local - s * y_local + robot_x
            # y_world = s * x_local + c * y_local + robot_y
            
            world_traj = np.zeros_like(local_traj_np)
            world_traj[:, 0] = c * local_traj_np[:, 0] - s * local_traj_np[:, 1] + current_pos_world[0]
            world_traj[:, 1] = s * local_traj_np[:, 0] + c * local_traj_np[:, 1] + current_pos_world[1]
...
class RobotVisualEnv(gym.Env):
    ...
    def __init__(self, model_path, low_level_policy_path, render_mode='rgb_array', 
        ...
        # Diffusion planner
        self.use_diffusion_planner = use_diffusion_planner
        if self.use_diffusion_planner:
            self.diffusion_planner = DiffusionTrajectoryPlanner(
                device="cuda" if torch.cuda.is_available() else "cpu",
                checkpoint_path=diffusion_model_path,
                horizon=20 # 生成未来20个点的轨迹
            )
        # -------------------------------
    ...
    def _update_path(self):
        ...
        if self.use_diffusion_planner:
            # 获取当前的 Yaw 角
            _, robot_yaw = self._get_robot_pose()
            
            # 获取局部地图
            local_map = self.grid_map.get_local_map(robot_pos, robot_yaw)
            
            diffusion_path = self.diffusion_planner.plan(
                local_map_np=local_map, 
                current_pos_world=robot_pos, 
                current_yaw=robot_yaw,
                goal_pos_world=self.goal_pos
            )
            
            self.current_path = diffusion_path
